...
newSearch={
	...
	noLogin:function(){
		...
		*/
		...
		if($echoUserID==0){ 
			$.ajax({
				url:'//search.baihe.com/search/noLogin?&jsonCallBack=?', 
				type:'post',
				dataType:'jsonp',
				async : true,
				success:function(data){
					if(data.state!=-1){
						...
						window.location.href='//my.baihe.com/login/?ReturnUrl=https://search.baihe.com/';
					}
				}
			});
		}else{
	...
	noLoginCon:function($data,$num){
		...
		for(var i=0;i<$data.length;i++){
			...
			html+="		<a href='//profile1.baihe.com?oppID="+$data[i].userID+"' target='_blank' class='pic'>";
			html+=$data[i].gender==1?"<img data-original='"+$data[i].headPhotoUrl+"' height='239' src='//images6.baihe.com/icon/loadingS.gif' alt='' /><span class='online' style='display:none;'>在线</span><em style='display:none;' onclick='todayStar()' class='today-star'>今日明星</em>":"<img  data-original='"+$data[i].headPhotoUrl+"' src='//images6.baihe.com/icon/loadingS.gif' height='239' alt=''/><span class='online' style='display:none;'>在线</span><em style='display:none;' onclick='todayStar()' class='today-star'>今日明星</em>";
			html+="		</a>";
			html+="		<div class='txt'>";
			html+="			<a href='//profile1.baihe.com?oppID="+$data[i].userID+"' target='_blank' class='hy'>"+$data[i].nickname+"</a>";
			html+="			<p>"+$data[i].age+"岁<span>|</span>"+$data[i].height+"cm<span>|</span>"+$data[i].educationChn+"<span>|</span>"+$data[i].incomeChn+"</p>";
			html+="		</div>";
			html+="		<div class='icon'>";
			html+="			<a class='say sayHello' event='3' spm='4.4.1219.971.11505' href='//profile1.baihe.com?oppID="+$data[i].userID+"' target='_blank'>打招呼</a>";
			html+="			<a class='send' event='3' spm='4.4.1219.970.11506' href='//profile1.baihe.com?oppID="+$data[i].userID+"' target='_blank'>发消息</a>";
			html+="		</div>";
			html+="	</div>";
			html+="</li>";
	...
	getSelf:function(){
		var _this = this;
		$.ajax({
			url:'//search.baihe.com/search/getUserList?userIDs='+$echoUserID+'&jsonCallBack=?', 
			type:'post',
			async : true,
			dataType:'jsonp',
			success:function(data){
	...
	getPerson:function(){
		var _this = this;
		$.ajax({
			url:'//search.baihe.com/Search/getUserID?&jsonCallBack=?', 
			type:'post',
			data:basicParams,
			async : true,
			dataType:'jsonp',
	...
	getInfo:function($data,$num){
		var _this=this;
		$.ajax({
			url:'//search.baihe.com/search/getUserList?userIDs='+$data+'&jsonCallBack=?', 
			type:'post',
			dataType:'jsonp',
			async : true,
			success:function(data){
	...
	getInfoContent:function($data,$num,i){		
		var html='';
			...
			html+="	<div class='memberS'>";
			html+="		<a href='//profile1.baihe.com?oppID="+$data.userID+"' target='_blank' class='pic'>";
			html+=$data.gender==1?"<img data-original='"+$data.headPhotoUrl+"' src='//images6.baihe.com/icon/loadingS.gif' alt='' height='239'/><span class='online' style='display:none;'>在线</span><em style='display:none;' onclick='todayStar()' class='today-star'>今日明星</em>":"<img  data-original='"+$data.headPhotoUrl+"' src='//images6.baihe.com/icon/loadingS.gif' height='239' alt=''/><span class='online' style='display:none;'>在线</span><em style='display:none;' onclick='todayStar()' class='today-star'>今日明星</em>";
			html+="		</a>";
			html+="		<div class='txt'>";
			html+="			<a href='//profile1.baihe.com?oppID="+$data.userID+"' target='_blank' class='hy'>"+$data.nickname+"</a>";
			html+="			<p>"+$data.age+"岁<span>|</span>"+$data.height+"cm<span>|</span>"+$data.educationChn+"<span>|</span>"+$data.incomeChn+"</p>";
			html+="		</div>";
			html+="		<div class='icon'>";
			html+=$num=='1'?"<a href='javascript:;' class='say sayHello' onclick='greet2(\""+$data.userID+"\",\"02.00.10404\",true); baihe.bhtongji.tongji({\"event\": \"3\",\"spm\": \"4.4.1158.971.11522\"})'>打招呼</a>":"<a href='javascript:;' class='say sayHello' onclick='greet2(\""+$data.userID+"\",\"02.00.10401\",true); baihe.bhtongji.tongji({\"event\": \"3\",\"spm\": \"4.4.1219.971.11505\"})'>打招呼</a>";
	...
	starToday:function(){
		...
		if($echoUserID!=0){
			...
			var $yu=[];
			$.ajax({
				url:'//search.baihe.com/Search/todayStarList?&jsonCallBack=?', 
				type:'post',
				async : true,
				dataType:'jsonp',
				success:function(data){ 
	...
	clickStar:function($data){
		...
		$('.starsMore').click(function(){
			...
			$('.stars').html('').height(358);
			//剩余的userID数组
			$yuData=_this.startArr.slice(4);
			//截取4个传入接口 展示第一屏数据
			var $data1=_this.startArr.splice(0,4);
			if($yuData.length>0){
				//根据userID 取用户资料
				_this.getInfo($data1,'1');
			}else{
...
// 今日明星跳转;
function todayStar(e){
	window.location.href='//product.baihe.com/Viptodaystar';
...
function openMember(){
	...
	var dhtml = '';
		...
		dhtml += '	<a href="//product.baihe.com/vipcrystal" target="_blank" class="icon openMember">开通会员</a>';
		dhtml += '</div>';
...
function uploadPhotos(){
	...
	var dhtml = '';
		...
		dhtml += '	<a href="//my.baihe.com/myphoto/uploadPhotos" target="_blank" class="icon uploadImg">上传照片</a>';
		dhtml += '</div>';
...
function signIn(){
	var dhtml = '';
		...
		dhtml += '	<a href="//my.baihe.com/login/" class="icon">我要登录</a>';
		dhtml += '</div>';
...
function errorImg(img,$data){
	(function(root) {        
		var x = root.xlk  || {};
			if($data.gender==1){
				x.IMG_DEFAULT_SRC = "//images7.baihe.com/pic/nopic_male.jpg";
			}else{
				x.IMG_DEFAULT_SRC = "//images7.baihe.com/pic/nopic_female.jpg";
			}
